...
datablockhdr* cellpoolAN::alloc_block(int cells) {
	// allocate a full block
	assert(cells > 0);
	datablockhdr *block = (datablockhdr *)_mm_malloc(sizeof(datablockhdr) + sizeof(CellAN)*cells, c_cacheline_size);

	// initialize header and cells
	block->next = NULL;
	CellAN *temp1 = (CellAN *)(block+1);
...
void cellpoolAN::initialize(int particles) {
	...
	cells = (CellAN *)(datablocks+1);
}

// Destroy the memory pool
cellpoolAN::~cellpoolAN() {
	while(datablocks) {
...
void cellpoolAN::return_cell(CellAN *cell) {
	assert(cell);
	cell->next = cells;
	cells = cell;
}
